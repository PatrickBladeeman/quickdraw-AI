...
def validate_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if contract["runtime"] != _runtime_contract():
        raise LLAPIContractError("The active runtime differs from R3K.")
    pyproject = tomllib.loads(PYPROJECT_PATH.read_text(encoding="utf-8"))
    if pyproject["project"]["name"] != contract["package"]["distribution"]:
        raise LLAPIContractError("The R3K package name has drifted.")
    if pyproject["project"]["version"] != contract["package"]["version"]:
        raise LLAPIContractError("The R3K package version has drifted.")
...
def run_fresh_worker(
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"Fresh R3K worker {worker_index + 1} failed with exit code "
            f"{completed.returncode}; see {log_path}."
        )
    trace_path = worker_output / TRACE_FILE_NAME
    if not trace_path.is_file():
        raise RuntimeError(f"Fresh R3K worker omitted {trace_path}.")
...
def main() -> int:
    ...
    if first != second or first_path.read_bytes() != second_path.read_bytes():
        raise LLAPIContractError(
            f"Fresh R3K traces differ: {first_path} versus {second_path}."
        )
