...
def test_r3k_contract_rejects_a_drifted_r3j_binding() -> None:
    drifted = copy.deepcopy(_contract())
    drifted["base_scheduled_handoff_contract"]["sha256"] = "0" * 64
...
def _set_transition_limit(contract: dict[str, Any]) -> None:
    contract["collection"]["transition_limit"] = 10_009
...
def _select_after_update(contract: dict[str, Any]) -> None:
    contract["third_update_boundary"]["select_action_after_third_update"] = True
...
def test_r3k_contract_schema_rejects_boundary_drift(
...
def test_r3k_result_schema_freezes_the_no_handoff_cutoff() -> None:
    schema = json.loads(RESULT_SCHEMA_PATH.read_text(encoding="utf-8"))
    trace = schema["$defs"]["trace"]
    selector = trace["properties"]["selector"]["properties"]

    assert trace["properties"]["transitions"]["minItems"] == 10_008
    assert trace["properties"]["transitions"]["maxItems"] == 10_008
    assert trace["properties"]["optimization"]["properties"][
        "optimizer_update_count"
    ] == {"const": 3}
    assert selector["selection_count"] == {"const": 10_008}
...
def test_r3k_task_trace_validation_preserves_prefix_and_update_3(
    ...
    monkeypatch.setattr(
        "run_bdq_third_update_smoke.validate_update_gate_trace",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        "run_bdq_third_update_smoke.canonical_json_sha256",
        lambda value: contract["r3j_prefix"]["canonical_transitions_sha256"],
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda trace: trace["optimization"]["update_events"][2].__setitem__(
                "online_after_sha256",
                trace["optimization"]["update_events"][1][
                    "online_after_sha256"
                ],
            ),
            "did not change",
...
def test_r3k_task_trace_validation_rejects_drift(
    monkeypatch: pytest.MonkeyPatch,
    mutate: Callable[[dict[str, Any]], None],
    message: str,
) -> None:
...
def test_r3k_cli_separates_parent_and_worker_modes() -> None:
    parent = parse_arguments(
        ["--env", "player.exe", "--output", "r3k-acceptance"]
    )
