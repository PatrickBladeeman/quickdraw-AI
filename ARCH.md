...
Research_Basic (implemented R2A environment benchmark)
  84×84 grayscale frame stack
    → random/scripted baseline or R3D-R3M direct LLAPI BDQ action selector
      (R3A tensors + R3B optimizer + direct replay and four live updates implemented;
...
- `Research/trainer/quickdraw_bdq` — R3A replay/action/network/target foundation,
  ...
  R3M scheduled fourth-update gate.
...
Use Double-DQN online action selection and target-network evaluation, replay capacity 100,000, replay warmup 10,000 decisions, batch 64, `gamma=0.99`, Adam `1e-4`, Huber loss averaged over selected branch values, update every four decisions, and hard target synchronization every 10,000 optimizer updates. Training epsilon stays at `1.0` through 10,000 completed transitions, decays linearly to `0.1` across the next 100,000 transitions, and remains clamped at `0.1`; the `0.05` floor is only for exploratory validation. Final evaluation is greedy.
...
Research/                                (non-Unity research source and contracts)
  ...
  trainer/                               (R3A foundation through R3M update gate)
  analysis/                              (planned)
