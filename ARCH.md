...
Research_Basic (implemented R2A environment benchmark)
  84×84 grayscale frame stack
    → random/scripted baseline or R3D-R3N direct LLAPI BDQ action selector
      (R3A tensors + R3B optimizer + lossless bounded replay and four live updates implemented;
       R3I schedule unit-tested; R3J live handoff and R3K/R3M updates verified;
       extended learning rollout not implemented)
...
- `Research/trainer/quickdraw_bdq` — R3A replay/action/network/target foundation,
  ...
  gate, R3G second-update gate, R3H post-update handoff gate, R3I stateless
  epsilon schedule plus scheduled selector, R3J bounded live schedule gate, and
  R3K scheduled third-update gate, R3L diagnostic greedy-handoff gate, and
  R3M scheduled fourth-update gate, and R3N lossless memory-bounded replay.
  The former ML-Agents trainer, policy,
  trajectory, settings,
  plugin, and next-mask-registry abstractions are retired. R3M extends the
  production schedule only through its exact 10,012-transition fourth-update
  smoke; R3N changes only replay storage representation and preserves that
  boundary exactly;
  extended training and checkpoint/export remain closed.
...
`ResearchBasicEpisode` is a Unity-independent deterministic state machine. It owns the explicit scenario seed and zero-based episode index, nine-slot target sampler, position, ammunition, cooldown, counters, additive reward, `target_hit` terminal, and `decision_limit` truncation. It fails closed if decision ordering, semantic actions, physical hitscan results, or post-end continuation disagree with the contract. Immediately before `EpisodeInterrupted()`, the agent publishes one Unity-to-Python `quickdraw.basic-truncation-mask.v1` message with the final position and environment-authored unavailable-action masks. The episode exposes legality for that final continuation state even though normal post-end actions remain disabled.
...
### Branching Double DQN (BDQ) foundation, optimizer, and direct collection boundary

Unity uses ML-Agents `4.0.0` for sensors, branched action transport, episode boundaries, and later exported-model inference. The Python side pins Python `3.11.13`, ML-Agents Envs `1.1.0`, NumPy `1.23.5`, and PyTorch `2.12.0+cpu`, using the already validated metadata-overlay compatibility lane. R3B historically proved official plugin discovery, but the current package intentionally registers no `mlagents.trainer_type` entry point. `UnityEnvironment` is driven synchronously: one pending decision is completed when the same agent next appears in `DecisionSteps` or `TerminalSteps`, producing exactly one immutable replay transition. Built-in PPO/SAC configuration is not a substitute for BDQ.
...
Use Double-DQN online action selection and target-network evaluation, replay capacity 100,000, replay warmup 10,000 decisions, batch 64, `gamma=0.99`, Adam `1e-4`, Huber loss averaged over selected branch values, update every four decisions, and hard target synchronization every 10,000 optimizer updates. Replay exposes exact C-contiguous float32 HWC `[84,84,4]` observations while storing content-addressed raw float32 channel frames and preallocated columnar transition metadata. Reference counts reclaim frames after ring overwrite. Accounted storage is capped at 4 GiB and insertion fails atomically when a lossless stream cannot fit; the ceiling does not include caller-owned transitions, sampled batches, networks, or optimizer state. Training epsilon stays at `1.0` through 10,000 completed transitions, decays linearly to `0.1` across the next 100,000 transitions, and remains clamped at `0.1`; the `0.05` floor is only for exploratory validation. Final evaluation is greedy.

R3A implements and tests the immutable replay record/buffer, exact visual network and dueling recombination, legal branch selection, reversible six-action mapping, per-branch Double-DQN target construction, and mean branch Huber loss. True `target_hit` terminals do not bootstrap; registered decision-limit truncations do. R3B adds exact seeded online/target initialization, Adam updates, replay/update gates, metrics, and post-update hard synchronization. The direct collector validates exact `QuickDrawResearchBasic?team=0` behavior, HWC observation and `[3,2]` branch shapes, preserves ML-Agents' `true = unavailable` mask meaning, and holds only the data needed to complete the pending action. Ordinary continuation gets its next mask from the next `DecisionStep`; a true terminal uses an irrelevant all-available sentinel. Unity sends an authoritative final-state action mask immediately before a decision-limit interruption, so truncated replay can bootstrap without Python inspecting scene state. R3D's verified two-process smoke inserted 302 transitions per process: a two-step online-network terminal episode plus a 300-step truncation at slot `-4` with final movement mask `[false,true,false]`. R3E adds a fixed seeded epsilon-greedy selector and exact clean cutoff: each fresh process collected 1,000 legal transitions across 18 resets, exercised all six action tuples, and ended with no pending action. Both gates performed no update or target sync, preserved all weights, and repeated byte-for-byte. R3F reaches the exact production warmup with 10,000 real Unity transitions and opens one batch-64 Adam update at decision 10,000. Two fresh processes produced byte-identical traces, finite loss and TD-error metrics, the same changed online hash, the same frozen target hash, zero target synchronizations, and no pending decision. Its separate single-run watch path can connect to the Unity Editor on port 5004 or launch a visible standalone player at time scale 1 while reporting terminal progress; it writes one diagnostic trace and cannot produce acceptance evidence. R3G extends the same seeded trace by exactly four completed transitions and proves the recurring update at decision 10,004. R3H preserves that complete prefix, evaluates the next live observation through the twice-updated online network and frozen target, selects the legal masked online argmax, completes exactly one more Unity transition, and stops before a third update. Two fresh R3H processes repeat the action, Q-value evidence, and complete trace byte-for-byte. R3I adds the hash-bound, stateless production epsilon schedule and a seeded scheduled selector; 30 focused tests cover its exact boundaries, mask safety, repeatability, fast path, and rejection behavior. R3J replays the same 10,004-transition prefix through one continuous scheduled selector, observes the exact count-derived epsilon values, completes legal exploratory action `[0,0]` at epsilon `0.999964`, and repeats the complete 10,005-transition trace byte-for-byte across two fresh processes. The R3J action is exploration evidence, not evidence that updated weights chose it. R3K preserves all 10,005 R3J transitions, consumes scheduled selections only at counts 10,005 through 10,007, and completes optimizer update 3 at decision 10,008 before selecting another action. Two fresh R3K processes repeat the complete trace byte-for-byte, update the online hash to `4f78e397e87ad6cea1ada78d49dea808337c401f6478970d1a4439065743775b`, keep the target frozen, and stop without a pending decision. R3L preserves that entire scheduled prefix, does not consume the production selector at count 10,008, and instead performs one explicit epsilon-zero masked-greedy evaluation with both the update-3 online network and frozen target. It selects `[2,1]`, records the full branch Q arrays and a maximum difference of `0.08219506591558456`, completes transition 10,009, and stops before update 4. Two independently completed fresh-process traces from separate parent attempts match byte-for-byte; a recovery comparison path validates both full traces and cannot count failed or partial workers. R3L is live weight-to-action integration evidence, not extended training or policy-effectiveness evidence. R3M returns to R3K's uninterrupted production trajectory rather than continuing R3L's diagnostic action. It preserves all 10,008 R3K transitions and optimizer events, consumes scheduled selections at counts 10,008 through 10,011, and completes update 4 exactly at decision 10,012 with loss `0.0085072573274374`, mean absolute TD error `0.06249994412064552`, and online hash `a8356df1531b99a42966578c6fd784cd384c8bcd3d3c3092124df13b2587268f`. Two fresh traces are byte-identical, the target remains frozen, and no decision remains pending. The standalone player is now configured to continue while unfocused; exact prefix validation proves that operational setting did not change accepted observations. R3N stores each exact raw float32 channel frame once, keeps transition metadata in fixed NumPy columns, and reconstructs the unchanged public batches. At capacity, the old observation payload alone would require 21.03 GiB; the registered Basic state bound accounts for 12,037,184 bytes and a conservative sequential projection for 2,934,585,088 bytes under the fail-closed 4 GiB limit. One fresh R3M trace matches the frozen pre-R3N serialized and canonical hashes exactly, including all transitions and four optimizer results. Extended epsilon-decay rollout, update 5, target synchronization, extended training, ROCm training, checkpoint/resume, ONNX export, and effectiveness claims remain unimplemented.
...
Research/                                (non-Unity research source and contracts)
  ...
  trainer/                               (R3A foundation through R3N replay gate)
  analysis/                              (planned)
