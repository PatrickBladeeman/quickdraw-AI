...
Research_Basic (implemented R2A environment benchmark)
  84×84 grayscale frame stack
    → random/scripted baseline or R3D-R3J direct LLAPI BDQ action selector
      (R3A tensors + R3B optimizer + direct replay and two live updates implemented;
       R3I schedule unit-tested; R3J bounded live schedule handoff verified;
...
- `Research/trainer/quickdraw_bdq` — R3A replay/action/network/target foundation,
  ...
  epsilon schedule plus scheduled selector, and R3J bounded live schedule gate.
  The former ML-Agents trainer, policy,
  trajectory, settings,
  plugin, and next-mask-registry abstractions are retired. R3J connects the
  schedule only to its exact 10,005-transition integration smoke; extended
  training and checkpoint/export remain closed.
...
Research/                                (non-Unity research source and contracts)
  ...
  trainer/                               (R3A foundation through R3J live schedule gate)
  analysis/                              (planned)
