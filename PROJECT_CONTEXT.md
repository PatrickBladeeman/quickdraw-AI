...
- Multi-environment Unity collection, an extended environment learning loop or epsilon-decay rollout, checkpoint/export lifecycle, learned-policy evaluation, or trained policies. R3L stops after one explicitly diagnostic update-3 masked-greedy transition; it does not open update 4 or an extended rollout. R1F accepts ROCm only for its fixed-policy inference fixture.
