...
- Multi-environment Unity collection, an extended environment learning loop or epsilon-decay rollout, checkpoint/export lifecycle, learned-policy evaluation, or trained policies. R3J connects the schedule only to a bounded 10,005-transition integration gate and does not open a third update. R1F accepts ROCm only for its fixed-policy inference fixture.
