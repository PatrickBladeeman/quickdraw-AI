...
Task 8 is committed and pushed as two consecutive Task 8-only commits: `ac1274e` (`task 8 : Logging and telemetry`) and `b5532e1` (`task 8 continued`). R1A was committed and pushed as `7117b81`; R1B as `4863153`; R1C as `5bae5f1`; revised R1E plus cleanup as `6d2aa3b`; Bryan's R1F commit as `876cedd`; R2A as `3cf3494` (`R2A: add deterministic Basic visual baseline`); the gradual-motion roadmap update as `3427c14`; R3A as `20aeab7` (`R3A: BDQ contract and schema`); its follow-up documentation synchronization as `7df359e`; and R3B plus the bounded R2A visual/lifecycle maintenance as `500bc24` (`add optimizer implementation and unity cli`). Those commits are pushed. R3D direct-LLAPI/truncation-mask collection is implemented and verified in the current working tree but is not committed or pushed. Current local gates pass all 31 Edit Mode tests, all 56 Python research tests, a clean saved-scene Windows build, and the exact two-process 302-transition direct-LLAPI comparison.
...
- Replay-warmup-scale or multi-environment Unity collection, an operational environment learning loop, epsilon-decay rollout, checkpoint/export lifecycle, learned-policy evaluation, or trained policies. The direct smoke deliberately performs no learning. R1F accepts ROCm only for its fixed-policy inference fixture.
...
Research Tasks R0, R1A, R1B, R1C, revised R1E, R1F, the R2A environment-only slice, R3A, and R3B are complete and pushed; the bounded R2A visual/lifecycle maintenance is included in `500bc24`; the exploratory R1D lane is superseded and retired. R3C records the uncommitted, superseded high-level trainer/trajectory experiment. R3D direct-LLAPI/truncation-mask collection is implemented and verified locally but not yet committed or pushed. R3A freezes tensor/replay meanings, R3B supplies deterministic optimization, and R3D proves terminal and bootstrapped-truncation replay with exact fresh-process repeatability and no weight change. R3E is the next bounded SSNT: fixed, seeded epsilon-greedy direct collection of exactly 1,000 transitions below replay warmup, with no optimizer update, target synchronization, checkpoint, or learned-policy claim. Checkpoint/export, learned-policy evaluation, strategic combat, and LLM runtime remain unimplemented and require separately approved work. After the full R3 Q-learning acceptance gates pass, the separately versioned gradual-motion Basic variant remains planned; it must preserve slot-based R2A as the control and use its own training/evaluation.
