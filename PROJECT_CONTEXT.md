...
Task 8 is committed and pushed as two consecutive Task 8-only commits: `ac1274e` (`task 8 : Logging and telemetry`) and `b5532e1` (`task 8 continued`). R1A was committed and pushed as `7117b81`; R1B as `4863153`; R1C as `5bae5f1`; and revised R1E plus cleanup as `6d2aa3b`, which is the current `main`, `origin/main`, and `origin/HEAD`. R1F is implemented and verified locally but remains uncommitted. Research Tasks R0, R1A, R1B, R1C, R1E, and R1F are complete at the working-tree level. R1F passed every exact trace/action/outcome/checkpoint/ONNX gate and the `1e-5` logit tolerance. CPU throughput was 81.321 and 105.108 decisions/s (median 93.2145); ROCm was 96.430 and 100.213 (median 98.3215). The frozen all-or-nothing rule therefore accepts ROCm for this batch-size-one synchronous inference fixture, not for training or model quality.

Not implemented:

- Basic or strategic research scenes, visual/runtime implementations of their frozen benchmark episode/reward/reset/observation/action/terminal/truncation contracts, BDQ training, replay, learned-policy export, or trained policies. R1F accepts ROCm only for the registered fixed-policy inference fixture.
...
Research Tasks R0, R1A, R1B, R1C, revised R1E, and R1F are complete; the exploratory R1D lane is superseded and retired. R1C established the exact 10,000-decision CPU transport reference. R1E established the isolated Python 3.11.13 / ML-Agents 1.1.0 / ROCm 7.14.0 compatibility lane on the exact RX 7900 XT. R1F created one seeded fixed float32 policy, one checkpoint, and one ONNX export, then ran alternating CPU-1, ROCm-1, CPU-2, ROCm-2 evidence with a real 1,000-decision warmup and 10,000 measured decisions per run. Every exact and tolerance correctness gate passed; CPU-versus-ROCm and ONNX-versus-CPU maximum absolute errors were `9.5367431640625e-07`. CPU median throughput was `93.2145` decisions/s and ROCm median was `98.3215`, so the frozen all-or-nothing rule accepts ROCm for this fixture. The next separately approved candidate is R2A: implement and test only the deterministic `Research_Basic` scene, observation, action, episode-end, and reset contract without starting BDQ training or LLM runtime work.
