...
- R3L's hash-bound `quickdraw.bdq-third-update-greedy-handoff.v1` gate: the exact 10,008-transition R3K scheduled prefix and all three optimizer events remain unchanged. At count 10,008, R3L deliberately does not consume production epsilon `0.999928`; it evaluates the live observation with epsilon `0.0`, selects legal masked online argmax `[2,1]`, completes transition 10,009, and stops before update 4 with the target frozen and no pending decision. The maximum online-versus-target Q delta is `0.08219506591558456`. Two independently completed fresh-process traces from separate parent attempts are object-equal and raw-byte-equal with serialized SHA-256 `1334f396fa0445cf4c4563c28ef22e8fa8cfc8ce383687b3c2918d1515bc4665`; failed or partial Unity workers do not count. All 22 focused R3L cases, all 153 trainer tests, and all 170 Python research tests pass. This proves update-3 weights reach live selection, not that update 3 changes the selected action or that the policy is effective.
- R3M's hash-bound `quickdraw.bdq-fourth-update.v1` gate returns to R3K's uninterrupted production trajectory and explicitly excludes R3L's diagnostic transition. It preserves all 10,008 R3K transitions and the first three optimizer events, consumes scheduled selections at counts 10,008 through 10,011 with epsilons `0.999928`, `0.999919`, `0.99991`, and `0.999901`, and completes optimizer update 4 exactly at decision 10,012. Update 4 has loss `0.0085072573274374`, mean absolute TD error `0.06249994412064552`, and online hash `a8356df1531b99a42966578c6fd784cd384c8bcd3d3c3092124df13b2587268f`; the target remains frozen, target synchronization stays at zero, no decision remains pending, and no post-update action is selected. Two fresh Python/Unity processes each complete 215 episodes and three truncations and produce object-equal, raw-byte-equal traces. All 15 focused R3M cases, all 168 trainer tests, all 185 Python research tests, and all 31 Unity EditMode tests pass. The standalone player now runs while unfocused; exact R3K-prefix validation proves this operational setting did not alter accepted observations. Five older-player attempts timed out, and a completed `-batchmode` trace was rejected for camera-observation drift at transition 0. This proves only the bounded fourth scheduled update, not extended training or policy effectiveness.
- R3N's hash-bound `quickdraw.bdq-replay-storage.v1` gate replaces the separate full float32 `s`/`s'` arrays—which would consume 21.03 GiB at capacity before Python overhead—with exact content-addressed float32 channel frames and preallocated columnar metadata. The unchanged public replay API still returns C-contiguous HWC `[84,84,4]` float32 batches and preserves ring indices, fields, and seeded sample draws. Reference counts reclaim orphaned frames; insertion is transactional and fails closed against a 4 GiB accounted-storage ceiling. Registered Basic projects to `12,037,184` bytes and a conservative 100,004-distinct-frame stream to `2,934,585,088` bytes. One fresh post-R3N R3M run reproduces the frozen 10,012-transition serialized and canonical trace hashes, all four optimizer results, all online hashes, and the frozen target exactly. Its conservative live upper bound is `190,888,704` bytes. All 12 focused R3N cases, all 180 trainer tests, all 197 Python research tests, and all 31 normal-graphics Unity EditMode tests pass. A first invalid `-nographics` Unity invocation produced the expected camera-device failure; the normal-graphics rerun passed without a code or threshold change. This is lossless storage and regression evidence, not a benchmark or effectiveness study.
...
R3K is committed and pushed as `2960e5b` (`R3K: 3rd optimizer update`). Its contract SHA-256 is `930839d2a19022b6debc0ad48cfc7f64cd86ff948be2690a9989a48598cda528`; canonical trace SHA-256 is `c3fafe259dfc03d69e06c758af0c2ba4df3b5da3322d325a5e361cdcf3a4ff02`; and both serialized worker traces have SHA-256 `4e4733dd5e5cbfbd7daa21f6d2ad8c48981b4369769d8df000b709e9c115dde4`. R3K changes only the Python live-worker boundary, contract/schemas/runner/tests, and synchronized documentation; it changes no Unity source. Generated traces, player logs, and result evidence remain ignored.

R3L is committed and pushed as `f586507` (`R3K/L: greedy action with updated weights`). Its contract SHA-256 is `f56cfab90ba82e616e0e2be55f5d77d3262c98ac2dcc9bd604e735831f4b0ce6`; canonical trace SHA-256 is `19d5681bc78d8b1c7df0bef0e4ea3a5a32f757629428e11fa2961c5cafd581e3`; both successful serialized traces have SHA-256 `1334f396fa0445cf4c4563c28ef22e8fa8cfc8ce383687b3c2918d1515bc4665`; and the generated accepted result has SHA-256 `d53971c1155634722375cdc564f7c4a5207c2fa59b0d27c1abacd3d70de898ac`. R3L changes only the Python live-worker boundary, contract/schemas/runner/tests, and synchronized documentation; it changes no Unity source. Generated traces, player logs, and result evidence remain ignored.

R3M is committed and pushed as `2ec3ec3` (`R3M: 4th update tested with seeded episilon greedy`). Its contract SHA-256 is `0a6f209d93ef6d522a53f24807d4ce48e6e820a344905e9762583bbe13a72dbb`; canonical trace SHA-256 is `cd2fa70672432e74c16c34c0525035953961ac3845cbe3362e061373cf48a940`; both successful serialized traces have SHA-256 `762fd1d090dc88529f3ad86cd0ad87080aff6494587c10223ae89118bb910a51`; and the generated accepted result has SHA-256 `b79a1ba8ab0d23964c225a9b33236226b1c033f8e3de972d61e759f90d890d67`. The rebuilt player's `globalgamemanagers` SHA-256 is `23cce72e183cbb7c4c78850f1ba46d819e9d1bc60fbafbe970a112fa65cf4a47`. R3M changes the Python live-worker progress boundary, contract/schemas/runner/tests, synchronized documentation, and Unity's standalone background-execution setting; it does not change the Basic scene, action/reward contract, visual sensor, or dependencies. Generated builds, traces, player logs, and result evidence remain ignored.

R3N is implemented and verified locally but is not committed or pushed. Its contract SHA-256 is `ece35b60208adbb9ca0d36f8d61b3be652aae4a94ffdb7d2bcafc9ace58e16a9`. The fresh post-R3N trace retains serialized SHA-256 `762fd1d090dc88529f3ad86cd0ad87080aff6494587c10223ae89118bb910a51` and canonical SHA-256 `cd2fa70672432e74c16c34c0525035953961ac3845cbe3362e061373cf48a940`. R3N changes only Python replay storage, its public exports, contract/schema/tests/validator, and synchronized documentation; it changes no Unity source, dependencies, replay semantics, optimizer math, or training boundary. Generated builds, traces, and player logs remain ignored.
...
- Multi-environment Unity collection, an extended environment learning loop or epsilon-decay rollout, checkpoint/export lifecycle, learned-policy evaluation, or trained policies. R3M stops exactly at scheduled optimizer update 4; it does not open update 5, synchronize the target, or begin an extended rollout. R1F accepts ROCm only for its fixed-policy inference fixture.
