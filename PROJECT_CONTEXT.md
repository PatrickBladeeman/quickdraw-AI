...
- R2A's separate `Research_Basic` scene, typed action/actuator boundary, deterministic seeded episode state, actual uncompressed `[84,84,4]` HWC Rec. 601 grayscale stack, visible fixed reticle with the same center-camera hitscan, and schema-validated random/scripted LLAPI baselines.

Task 8 is committed and pushed as two consecutive Task 8-only commits: `ac1274e` (`task 8 : Logging and telemetry`) and `b5532e1` (`task 8 continued`). R1A was committed and pushed as `7117b81`; R1B as `4863153`; R1C as `5bae5f1`; revised R1E plus cleanup as `6d2aa3b`; Bryan's R1F commit as `876cedd`; and R2A as `3cf3494` (`R2A: add deterministic Basic visual baseline`), which is the current `main`, `origin/main`, and `origin/HEAD`. Research Tasks R0, R1A, R1B, R1C, R1E, R1F, and R2A are committed and pushed. R2A's final gates passed 29 Edit Mode tests, all 41 existing Play Mode tests, six Python tests, the isolated `Research_Basic` player build, the normal `Test_Arena` Windows build, and exact two-run standalone trace comparisons for both 12-episode random and scripted baselines. The scripted baseline succeeded in all 12 registered episodes; this validates mechanics and reproducibility, not learned-policy quality.

Not implemented:

- The strategic research scene, BDQ training, replay, learned-policy export, or trained policies. R1F accepts ROCm only for the registered fixed-policy inference fixture; R2A contains no training or learned weights.
...
Research Tasks R0, R1A, R1B, R1C, revised R1E, R1F, and the R2A environment-only slice are complete and pushed; the exploratory R1D lane is superseded and retired. R1C established the exact 10,000-decision CPU transport reference. R1E established the isolated Python 3.11.13 / ML-Agents 1.1.0 / ROCm 7.14.0 compatibility lane on the exact RX 7900 XT. R1F accepted ROCm for its fixed-policy inference fixture after every registered correctness and throughput gate passed. R2A implements and tests only the deterministic `Research_Basic` scene, visual observation, action, episode-end, reset, and random/scripted baseline contracts. BDQ/replay/training, learned model export, strategic combat, and LLM runtime work remain unimplemented and require a separately approved SSNT. After the R3 Q-learning acceptance gates pass, a separately versioned gradual-motion Basic variant is planned: it retains the discrete `[Stay, Left, Right]` action branch, holds lateral intent across fixed physics steps, preserves the slot-based R2A benchmark as the control, and requires its own training and evaluation rather than silently changing the canonical dynamics.
