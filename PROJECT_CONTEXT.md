...
- Unity ML-Agents `4.0.0`, its locked Inference Engine `2.2.1` dependency, and an isolated R1B Unity/Python communicator smoke fixture with five focused Edit Mode tests.

Task 8 is committed and pushed as two consecutive Task 8-only commits: `ac1274e` (`task 8 : Logging and telemetry`) and `b5532e1` (`task 8 continued`). The deterministic checkpoint remains `b5532e1`; R1A was committed and pushed as `7117b81`, which is the current `main`, `origin/main`, and `origin/HEAD`. R1B is implemented and verified locally but remains uncommitted. The checkpoint passed all 41 Play Mode tests and a Windows standalone build in Unity `6000.0.57f1`. Research Task R0, R1A contract/dependency preflight, and R1B package/communicator smoke are complete. R1B installed and locked ML-Agents `4.0.0`, completed two identical fixed-seed standalone traces containing both terminal and truncation paths, passed five focused Edit Mode tests plus all 41 existing Play Mode tests, and passed both isolated-smoke and normal Windows builds.

Not implemented:

- Basic or strategic research scenes, visual/runtime implementations of their frozen benchmark episode/reward/reset/observation/action/terminal/truncation contracts, the 10,000-step CPU/AMD gates, BDQ training, replay, model export, or trained policies. The implemented abstract smoke scene proves communication only.
...
Research Task R0, R1A, and R1B are complete. R1B installed and locked Unity ML-Agents `4.0.0`, added a separate abstract communicator fixture and strict side channel, and used the pinned Python 3.10.12 low-level API to complete two exactly matching base-seed-`21001` standalone traces. Each run exercised repeatable observations, action masks, actions, rewards, a `smoke_goal` terminal, a `decision_limit` truncation with `interrupted=true`, ordered side-channel events, and a schema-validated machine/hardware/run manifest. Five focused Edit Mode tests, all 41 existing Play Mode tests, the isolated smoke build, and the normal Windows build passed. No research benchmark, combat, trainer, model, AMD, or LLM implementation was added. The remaining R1 work still requires separate approval; the next candidate is R1C, limited to the registered deterministic 10,000-step CPU reference trace and throughput measurement.
