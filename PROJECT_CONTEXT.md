...
- Multi-environment Unity collection, an extended environment learning loop, epsilon-decay rollout, checkpoint/export lifecycle, learned-policy evaluation, or trained policies. R3H deliberately stops after completing one greedy action from the twice-updated online network. R1F accepts ROCm only for its fixed-policy inference fixture.
