...
Research Tasks R0, R1A, R1B, R1C, revised R1E, R1F, and the R2A environment-only slice are complete; the exploratory R1D lane is superseded and retired. R1C established the exact 10,000-decision CPU transport reference. R1E established the isolated Python 3.11.13 / ML-Agents 1.1.0 / ROCm 7.14.0 compatibility lane on the exact RX 7900 XT. R1F accepted ROCm for its fixed-policy inference fixture after every registered correctness and throughput gate passed. R2A implements and tests only the deterministic `Research_Basic` scene, visual observation, action, episode-end, reset, and random/scripted baseline contracts. BDQ/replay/training, learned model export, strategic combat, and LLM runtime work remain unimplemented and require a separately approved SSNT.
