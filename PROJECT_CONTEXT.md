...
Task 8 is committed and pushed as two consecutive Task 8-only commits: `ac1274e` (`task 8 : Logging and telemetry`) and `b5532e1` (`task 8 continued`). R1A was committed and pushed as `7117b81`; R1B as `4863153`; R1C as `5bae5f1`; revised R1E plus cleanup as `6d2aa3b`; Bryan's R1F commit as `876cedd`; R2A as `3cf3494` (`R2A: add deterministic Basic visual baseline`); the gradual-motion roadmap update as `3427c14`; R3A as `20aeab7` (`R3A: BDQ contract and schema`); its follow-up documentation synchronization as `7df359e`; R3B plus the bounded R2A visual/lifecycle maintenance as `500bc24` (`add optimizer implementation and unity cli`); R3D as `a4b449d` (`R3D: llapi implementation + truncation maski + pending decision`); and R3E as `6aef062` (`R3E: fixed episilon greedy test runs`). Those commits are pushed. R3E changes no Unity runtime or environment source.

R3F is implemented and verified in the current working tree but is not committed or pushed. Its contract SHA-256 is `edc2af190d778b9d43238a440e30c9ffd36a4b246a07933c03e233b3e2e94875`; canonical trace SHA-256 is `75c265377bbb171c24d817c7c45fb022e011b73fdb24749dd21c771ed436f08b`; and its two serialized worker traces share SHA-256 `aee36f7bc5c2e2202e2738709c021826878de717d22b91345001dafd8599f0b2`. One standalone watch validation also completed all 10,000 transitions at time scale 1 and produced the same serialized trace SHA-256 without a `result.json`; this verifies observability but is not additional acceptance evidence. R3F changes no Unity runtime or environment source.
...
- Multi-environment Unity collection, an extended environment learning loop, epsilon-decay rollout, checkpoint/export lifecycle, learned-policy evaluation, or trained policies. R3F deliberately stops after its first Unity-derived optimizer update. R1F accepts ROCm only for its fixed-policy inference fixture.
...
Research Tasks R0, R1A, R1B, R1C, revised R1E, R1F, the R2A environment-only slice, R3A, R3B, R3D, and R3E are complete and pushed; the bounded R2A visual/lifecycle maintenance is included in `500bc24`; the exploratory R1D lane is superseded and retired. R3C records the uncommitted, superseded high-level trainer/trajectory experiment. R3A freezes tensor/replay meanings, R3B supplies deterministic optimization, R3D proves terminal and bootstrapped-truncation replay with exact fresh-process repeatability and no weight change, and R3E proves fixed seeded epsilon-greedy collection across resets below warmup with no learning operation. R3F is implemented and verified locally: it reaches the exact 10,000-transition production warmup and performs one deterministic CPU update on real Unity experience while leaving the target frozen. Epsilon decay, a second live update, target synchronization, checkpoint/export, an extended training rollout, learned-policy evaluation, strategic combat, and LLM runtime remain unimplemented and require separately approved work. After the full R3 Q-learning acceptance gates pass, the separately versioned gradual-motion Basic variant remains planned; it must preserve slot-based R2A as the control and use its own training/evaluation.
