...
R3I is implemented and verified locally but is not committed or pushed. Its contract SHA-256 is `0d0f3f855ecf19a642f5dab2e526a1be2d2f0a1a87dca70671f4fad3128c3fa7`. R3I changes only the Python trainer package, contract/schema/tests, and synchronized documentation; it launches no Unity process and changes no Unity runtime or environment source.
...
- Multi-environment Unity collection, an extended environment learning loop, live epsilon-decay rollout, checkpoint/export lifecycle, learned-policy evaluation, or trained policies. R3I implements the stateless schedule but does not connect it to Unity; R3H remains the latest live transition. R1F accepts ROCm only for its fixed-policy inference fixture.
